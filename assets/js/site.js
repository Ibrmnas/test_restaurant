(function () {
  function token(str) {
    return (str || "")
      .replaceAll("{city}", window.SITE_CONFIG.city)
      .replaceAll("{brand}", window.SITE_CONFIG.brand);
  }
  ...
  function initLang() {
    ...
    var pref = saved
      ? saved
      : (navigator.language || "en").toLowerCase().startsWith("it")
      ? "it"
      : "en";
    setLang(pref);
  }
  ...
  function addToCart(key, qty) {
    ...
    if (found) {
      found.qty = (parseFloat(found.qty || 0) + qty);
    } else {
      var item = (window.SITE_CONFIG.items || []).find(function (i) {
        return i.key === key;
      });
      if (!item) return;
  ...
  function updateOrderCount() {
    var c = getCart().reduce(function (sum, x) {
      return sum + (parseFloat(x.qty) || 0);
    }, 0);
    var badge = document.getElementById("order-count");
    if (!badge) return;
    badge.textContent = c > 0 ? Math.round(c * 10) / 10 + " kg" : "";
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (itBtn) itBtn.addEventListener("click", function () { setLang("it"); });
  ...
  if (window.SITE_STATUS?.vacation) {
    ...
    //     a.setAttribute("href", window.SITE_CONFIG.wa);
    //   });
    // }
  });
})();
